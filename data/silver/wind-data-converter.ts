...
function findMinMax(arr: Array<number>) {
  let min = arr[0];
  let max = arr[0];
...
export async function jsonToProtobuf(
  jsonData: any,
  outputFile: string,
  options: { compress: boolean; quantize: boolean } = {
    compress: true,
    quantize: true,
  }
) {
  const { compress, quantize } = options;
...
export async function protobufToJson(
  inputFile: string,
  options: { compressed: boolean; quantized: boolean } = {
    compressed: true,
    quantized: true,
  }
) {
  const { compressed, quantized } = options;
